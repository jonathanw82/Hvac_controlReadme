...
function getAverageHistoricalData(url, measurement_type, length_of_hours, sets, chart){
...
function liveTempPChart(data){
  ...
  timeStamp.push(data.time_stamp.substring(11,16))
  if(dataObjNew.length > 122){
    dataObjNew.shift();
    timeStamp.shift();
  }
  window.tempPChart.update();
...
getAverageHistoricalData(URL_avgraph, 'temp_p_response', 2);
getAverageHistoricalData(URL_avgraph, 'temp_i_response', 2);
getAverageHistoricalData(URL_avgraph, 'hum_p_response', 2);
getAverageHistoricalData(URL_avgraph, 'hum_i_response', 2);
